...
录入快捷键时，点击可编辑项后按下目标按键或组合键，松开后完成录入。按 `Esc` 可取消本次录入。
...
请检查电脑是否已经联网，是否连接到局域网。网络刚切换时，可以点击刷新后再查看。
...
请在绘图软件中将输入 API 或平板设置切换为 **TabletPC / Windows Ink**。

这类问题常见于 CSP、SAI2 等默认使用 WinTab API 的软件。AirSlate PC Server 输出的是 Windows 笔输入事件，因此绘图软件需要使用 Windows Ink / TabletPC 输入路径才能正确接收压感。
...
可以直接重新录入。如果当前预设改得太多，也可以使用“恢复默认”。

### 可以同时连接多台鸿蒙设备吗？

当前建议同一时间只连接一台 AirSlate 设备。
